...
// Khai bao dong co buoc va chan se su dung
...
#define led 14 //khai bao chân den la 14 
int wheelSpeed = 150 ;
int lbw[50], lfw[50], rbw[50], rfw[50]; // Mang de luu vi tri va buoc
int servo1Pos, servo2Pos, servo3Pos, servo4Pos, servo5Pos, servo6Pos; // vi tri hien tai
int servo1PPos, servo2PPos, servo3PPos, servo4PPos, servo5PPos, servo6PPos; // vi tri truoc do
...
//int m = 0;
void setup() {
  // dat gia tri ban dau cho cac dong co buoc
  LeftFrontWheel.setMaxSpeed(300); // banh truoc trai 
  LeftBackWheel.setMaxSpeed(300);  // banh sau trai
  RightFrontWheel.setMaxSpeed(300);// banh truoc phai
  RightBackWheel.setMaxSpeed(300); // banh sau phai
  pinMode(led, OUTPUT);
  // khai bao chan cua cac servo
  servo01.attach(5); 
  servo02.attach(6);
  servo03.attach(7);
  servo04.attach(8);
  servo05.attach(9);
  servo06.attach(10);
  Bluetooth.begin(38400); // Toc do truyen mac dinh cua module bluetooch
  Bluetooth.setTimeout(5);
  delay(20);
  Serial.begin(38400); 
  // Di chuyen canh ta ve vi tri mac dinh 
  servo1PPos = 90 ;
  ...
  servo06.write(servo6PPos);
    ...
    // Di chuyen sang trai va giu nguyen huong ban dau
void moveSidewaysLeft() {
  LeftFrontWheel.setSpeed(-wheelSpeed); // wheelSpeed toc do da khai bao ben tren nhung then dau - de nguoc lai
  LeftBackWheel.setSpeed(wheelSpeed); // wheelSpeed toc do da khai bao ben tren 
  RightFrontWheel.setSpeed(wheelSpeed);
  RightBackWheel.setSpeed(-wheelSpeed);
}
  // Di chuyen sang phai va giu nguyen huong ban dau
...
void rotateRight() {
  ...
  // chay ve phia truoc theo huong lech phai
void moveRightForward() {
  ...
  // lui ve phia sau  theo huong lech phai
void moveRightBackward() {
  ...
  // chay ve phia truoc theo huong lech trai
void moveLeftForward() {
  ...
  // lui ve phia sau  theo huong lech trai
...
void runSteps() {
  while (dataIn != 13) {   // Run the steps over and over again until "RESET" button is pressed
    for (int i = 1; i <= index - 1; i++) {  // Run through all steps(index)
      if (Bluetooth.available() > 0) {      // khiem tra du lieu den 
        dataIn = Bluetooth.read();          // Doc du lieu nhan duoc
        if ( dataIn == 15) {           // neu "PAUSE" nhan
...
void loop() {
  ...
  if (Bluetooth.available() > 0) {
    dataIn = Bluetooth.read();  // doc xac nhan du lieu den
    if (dataIn == 0) {
      m = 0;
    }
    if (dataIn == 1) {
